# CLAUDE.md — ClassroomIO Fork (Contributor Workspace)

## Who I Am
I am a contributor working on a **personal fork** of [classroomio/classroomio](https://github.com/classroomio/classroomio). I did not build the original project. My changes will eventually be submitted as a PR to upstream. The goal is to keep changes clean, minimal, and mergeable.
...
### 1. Ask, Don't Assume
- If anything is unclear, **stop and ask immediately**. Do not fill gaps with assumptions.
- Do not infer intent from context. If I say "fix this," ask what "fixed" looks like before touching anything.

### 2. Stay On Track
- Work on **one task at a time**. Do not refactor, rename, or improve anything outside the current task scope.
- If you notice something unrelated that could be improved, **flag it as a note only** — do not act on it.
- If a change starts to grow beyond its original scope, stop and tell me.

### 3. Small, Targeted Changes Only
- Every change must be **as small as possible** while still completing the task.
- Prefer editing existing files over creating new ones unless explicitly needed.
- Do not restructure or reorganize files unless that is the task.

### 4. Maximize Token Efficiency
- Responses must be **short and direct**. No padding, no summaries of what you just did unless asked.
- Do not repeat back what I said. Skip preamble. Lead with the answer or output.
- When showing file edits, show only changed lines plus minimal surrounding context.

### 5. PR Safety
- Every change must be mergeable into upstream `main` without conflicts or confusion.
- Do not touch files outside the scope of the current task.
- Flag anything that might cause a merge conflict or deviate from upstream conventions.

---

## Project Overview

**Repo:** `classroomio/classroomio` (monorepo)
**Stack:** SvelteKit · Supabase · TailwindCSS · TypeScript · pnpm · Turborepo

### Apps in `/apps`
| App | Description | Local URL |
|---|---|---|
| `dashboard` | Main LMS web app | http://localhost:5173 |
| `classroomio-com` | Landing/marketing site | http://localhost:5174 |
| `api` | PDF, video, email/notification service | http://localhost:3002 |
| `docs` | Official documentation site | http://localhost:3000 |

---

## Local Dev Setup (Automated — PowerShell)

> ⚠️ This is **not** the upstream setup. I built a custom automated local environment using PowerShell scripts. It is the preferred setup for this fork and is actively being improved. I am open to adding new tools or dependencies to improve it even if upstream doesn't use them.

### Scripts (located in `/scripts`)
```powershell
# Full setup: installs prerequisites + runs repo dev setup
.\scripts\setup-machine.ps1

# Preview what will happen without making changes
.\scripts\setup-machine.ps1 -DryRun

# Install prerequisites only, skip repo/dev setup
.\scripts\setup-machine.ps1 -SkipDevSetup
...
- Checks/installs **NVM for Windows** and **Docker Desktop** via winget
- Installs and activates the correct **Node version** from `.nvmrc`
- Auto-starts Docker Desktop and waits for daemon to be ready
- Runs `corepack pnpm setup:dev` for repo dependency/dev setup
- Prompts to launch dev servers on completion

---

## Current Work Focus

**Active task:** Updating the README, developer setup instructions, and related documentation.

### Status
- Work in progress — not ready for upstream PR yet
- Changes will be reviewed and refined before submission

### What NOT to Do
- Do not modify application source code (Svelte components, API routes, DB schema) unless explicitly asked
- Do not update `pnpm-lock.yaml` or `package.json` without being asked
- Do not run `supabase db push` or any destructive DB commands without confirmation
- Do not suggest rewriting the automated setup in a different language — PowerShell is intentional
- Do not create documentation files speculatively

---

## Notes / To-Do
*(Update as work progresses)*
- [ ] Finalize and document `setup:dev` pnpm script behavior
- [ ] Define which README sections are being replaced vs. supplemented
- [ ] Determine final location/structure of `/scripts` folder for PR
